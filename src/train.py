...
class Experiment:
    ...
    def sup_simcse_collate_fn(self, data_list: list[dict]) -> BatchEncoding:
    ...
    def create_loader(
        ...
        if for_evaluation or self.args.method == "unsup-simcse":
            collate_fn = self.unsup_simcse_collate_fn
        elif self.args.method == "sup-simcse":
...
def main(args: Args):
    ...
    match args.method:
        case "unsup-simcse":
            train_dataset = UnsupSimCSEDataset(
                path=args.dataset_dir / args.dataset_name / "train.txt"
            )
        case "sup-simcse":
            train_dataset = SupSimCSEDataset(
                path=args.dataset_dir / args.dataset_name / "train.jsonl"
            )
        case _:
    ...
    with utils.ProgressBar(n=args.num_training_steps) as pbar:
        ...
        for epoch in count():
            ...
            for batch in train_dataloader:
                loss: torch.FloatTensor = train_step(batch.to(args.device))
